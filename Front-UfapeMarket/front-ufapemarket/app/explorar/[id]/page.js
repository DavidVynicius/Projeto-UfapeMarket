...
export default function DetalhesProdutoPage() {
        ...
        if (vendedorId) {
          fetch(`http://localhost:8081/produtos/vendedor/${vendedorId}`)
            ...
            .then((lista) => {
              setMaisDoVendedor(
                lista.filter(
                  (p) => Number(p.id || p.idProduto) !== Number(idProduto),
                ),
              );
            })
            .catch(() => {});
  ...
  if (carregando) {
    return (
      <div className="flex justify-center items-center min-h-[400px]">
        <p className="text-xs font-bold text-ink-faint animate-pulse">
  ...
  // TRATAMENTO DA DISPONIBILIDADE
  const qtdDisponivel = produto.quantidadeDisponivel ?? 0;
  const semEstoque = qtdDisponivel <= 0;
  const estaAtivo = !semEstoque && produto.disponivelParaVenda !== false;
  ...
  const qtdAvaliacoes =
    produto.quantidadeAvaliacoes || produto.vendedor?.quantidadeAvaliacoes || 0;
          ...
          <div className="bg-white p-3.5 rounded-2xl border border-line space-y-2 shadow-2xs">
            ...
            <div className="flex flex-wrap gap-2 pl-6">
              {listaPagamentos.map((pag, idx) => (
                <span
                  key={idx}
                  className="px-2.5 py-1 rounded-full bg-[#F1F3F4] text-[11px] font-bold text-[#3C4043]"
                >
                  {pag}
                </span>
              ))}
          ...
          <div className="bg-white p-3.5 rounded-2xl border border-line flex items-center justify-between shadow-2xs">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs font-bold text-[#111827]">
                  {produto.nomeVendedor || "Estudante UFAPE"}
                </p>
